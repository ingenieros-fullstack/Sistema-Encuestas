...
// ✅ Obtener encuesta asignada a un empleado
export const obtenerEncuestaEmpleado = async (req, res) => {
  try {
    const { codigo_formulario } = req.params;
    const id_usuario = req.user?.id_usuario; // ⚠️ Debe venir del JWT

    // Verificar asignación
    const asignacion = await Asignacion.findOne({ where: { id_usuario, codigo_formulario } });
    if (!asignacion) return res.status(403).json({ error: "Encuesta no asignada a este usuario" });
...
// ✅ Guardar respuestas de empleado
export const responderEncuestaEmpleado = async (req, res) => {
  try {
    const { codigo_formulario } = req.params;
