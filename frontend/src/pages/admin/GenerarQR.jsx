...
export default function GenerarQR() {
  ...
  const url = useMemo(() => `${window.location.origin}/resolver/${codigo}`, [codigo]);

  // Estado de personalización
  const [size, setSize] = useState(320); // 256–480 recomendado
  const [fg, setFg] = useState("#0f172a"); // color del QR
  const [bg, setBg] = useState("#ffffff"); // fondo
          ...
          <div className="qr-header-left">
            ...
            <div>
              ...
              <p className="qr-subtitle">
                {esEncuesta
                  ? "Escanee para acceder a la encuesta (empleados y supervisores)."
                  : "Escanee para acceder al cuestionario (empleados)."}
              </p>
            </div>
          </div>

          <div className="qr-badges">
            <span className="badge badge-dark">{esEncuesta ? "Encuesta" : "Cuestionario"}</span>
              ...
              <div className="field">
                ...
                <div className="color-row">
                  <input
                    ...
                    aria-label="Elegir color del QR"
                  />
                  <input
                    type="text"
                    value={fg}
