...
import "../../GestionSecciones.css"; // reutiliza el CSS del rediseño anterior

const API = import.meta.env.VITE_API_URL || "http://localhost:4000";

// Helpers para opciones "a;b;c"
    ...
    <div className="gs2-chips">
      {items.map((opt, i) => (
        <span key={i} className="gs2-chip">
          {opt}
          {onRemove && (
            <button className="x" type="button" onClick={() => onRemove(i)} aria-label="Quitar">
              ×
            </button>
          )}
        </span>
...
export default function GestionSeccionesCuestionario() {
  ...
  // editor nueva pregunta (compacto)
  ...
  useEffect(() => { fetchPreview(); /* eslint-disable-next-line */ }, [codigo]);
  ...
  const totalPreguntas =
    data?.Secciones?.reduce((acc, s) => acc + (s.Preguntas?.length || 0), 0) || 0;
  ...
  const crearSeccion = async () => {
    ...
    const res = await fetch(`${API}/admin/cuestionarios/secciones`, {
      ...
      body: JSON.stringify({ codigo_formulario: codigo, nombre_seccion: secNombre }),
    });
    if (res.ok) {
      setSecNombre("");
      await fetchPreview();
  ...
  const eliminarSeccion = async (id) => {
    ...
    if (res.ok) {
      if (String(id) === String(seccionSeleccionada)) setSeccionSeleccionada("");
      await fetchPreview();
    }
  };
  ...
  const guardarSeccionEditada = async () => {
    ...
    if (res.ok) { setEditSeccion(null); fetchPreview(); }
  };
  ...
  const crearPregunta = async () => {
    ...
    if (res.ok) { resetNuevaPregunta(); fetchPreview(); }
  };
  ...
  const guardarPreguntaEditada = async () => {
    const payload = { ...editPregunta, opciones: joinOptions(editPregunta.opciones) };
    const res = await fetch(
    ...
    if (res.ok) { setEditPregunta(null); fetchPreview(); }
  };

  // ======== Form de Pregunta (reutilizable y compacto) ========
  const PreguntaForm = ({ q, setQ, optInput, setOptInput }) => {
    ...
    const addOption = () => {
      ...
      setQ((prev) => ({ ...prev, opciones: [...prev.opciones, optInput.trim()] }));
      setOptInput("");
    };

    const removeOption = (i) => {
      setQ((prev) => ({ ...prev, opciones: prev.opciones.filter((_, x) => x !== i) }));
    };

    const setTipo = (tipo) => {
      setQ((prev) => ({ ...prev, tipo_pregunta: tipo, opciones: [], respuesta_correcta: "" }));
      setOptInput("");
    };

    const requiereOpciones =
      q.tipo_pregunta === "opcion_multiple" || q.tipo_pregunta === "seleccion_unica";
      ...
      <>
        <div className="gs2-grid">
          <div className="gs2-form-row">
            <label>Sección</label>
            <select className="gs2-input" value={q.id_seccion} onChange={handleChange("id_seccion")}>
        ...
        <div className="gs2-grid">
          ...
          <div className="gs2-form-row">
            ...
            <button
              ...
              onClick={() => setQ((prev) => ({ ...prev, obligatoria: !prev.obligatoria }))}
          ...
          <div className="gs2-block">
            <details className="gs2-details" open>
              ...
              <div className="gs2-inline">
                ...
                />
                <button type="button" className="gs2-btn gs2-btn-ghost" onClick={addOption}>
                  Añadir
                </button>
              </div>
    ...
    <div className="gs2-root">
      ...
      <div className="gs2-toolbar">
        <div className="gs2-toolbar-left">
          <a href={`/admin`} className="gs2-btn gs2-btn-ghost">← Inicio</a>
        </div>
        ...
        <div className="gs2-kpis">
          <div className="gs2-kpi"><div className="kpi-label">Secciones</div><div className="kpi-value">{totalSecciones}</div></div>
          <div className="gs2-kpi"><div className="kpi-label">Preguntas</div><div className="kpi-value">{totalPreguntas}</div></div>
          {selectedSection && (
            <div className="gs2-kpi hide-sm">
              <div className="kpi-label">Seleccionada</div>
              <div className="kpi-value">{selectedSection.nombre_seccion}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="gs2-mobile-tabs" role="tablist" aria-label="Navegación móvil">
        <button role="tab" aria-selected={mobileTab==="secciones"} className={"gs2-mtab "+(mobileTab==="secciones"?"active":"")} onClick={()=>setMobileTab("secciones")}>Secciones</button>
        <button role="tab" aria-selected={mobileTab==="preguntas"} className={"gs2-mtab "+(mobileTab==="preguntas"?"active":"")} onClick={()=>setMobileTab("preguntas")}>Preguntas</button>
        <button role="tab" aria-selected={mobileTab==="editor"} className={"gs2-mtab "+(mobileTab==="editor"?"active":"")} onClick={()=>setMobileTab("editor")}>Editor</button>
      </div>
      ...
      <div className="gs2-content" data-mobile-tab={mobileTab}>
        ...
        <aside className="gs2-pane pane-left">
          <div className="gs2-pane-header">
            ...
            <input className="gs2-input" placeholder="Buscar sección…" value={buscarSec} onChange={(e)=>setBuscarSec(e.target.value)} />
          </div>

          <div className="gs2-pane-body">
            <ul className="gs2-list">
              {data.Secciones?.filter(s => (s.nombre_seccion||"").toLowerCase().includes(buscarSec.toLowerCase()))
                .map((s)=> {
                  const active = String(s.id_seccion)===String(seccionSeleccionada);
                  return (
                    <li key={s.id_seccion} className={"gs2-list-item "+(active?"active":"")}>
                      <div className="gs2-list-text" onClick={()=>{ setSeccionSeleccionada(String(s.id_seccion)); setMobileTab("preguntas"); }}>
                        <div className="gs2-list-title">{s.nombre_seccion}</div>
                        <div className="gs2-list-sub">{s.Preguntas?.length || 0} pregunta(s)</div>
                      </div>
                      <div className="gs2-inline">
                        <button className="gs2-icon" title="Editar" onClick={()=>setEditSeccion(s)}>
                          {/* lápiz */}
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 20h9" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L8 18l-4 1 1-4 11.5-11.5Z" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        </button>
                        <button className="gs2-icon danger" title="Eliminar" onClick={()=>eliminarSeccion(s.id_seccion)}>
                          {/* papelera */}
              ...
              {!data.Secciones?.length && <li className="gs2-empty">Aún no hay secciones</li>}
            </ul>
          </div>
          ...
          <div className="gs2-pane-footer">
            <div className="gs2-form-row">
              ...
              <input className="gs2-input" placeholder="Ej. Cultura General" value={secNombre} onChange={(e)=>setSecNombre(e.target.value)} />
            </div>
            <button className="gs2-btn gs2-btn-primary w-100" onClick={crearSeccion} disabled={!secNombre.trim()}>
              ➕ Crear sección
            </button>
          </div>
        </aside>
        ...
        <section className="gs2-pane pane-middle">
          <div className="gs2-pane-header">
            ...
            <input className="gs2-input" placeholder="Buscar pregunta…" value={filtro} onChange={(e)=>setFiltro(e.target.value)} />
          </div>
                  ...
                  <li key={p.id_pregunta} className="gs2-q">
                    <div>
                      ...
                      <div className="gs2-q-meta">
                        ...
                        <span className="gs2-tag">puntaje {p.puntaje ?? 0}</span>
                        {p.respuesta_correcta && (
                          <span className="gs2-tag">correcta: {p.respuesta_correcta}</span>
                        )}
                      </div>
                    </div>
                    <div className="gs2-inline">
                      ...
                      <button className="gs2-icon danger" title="Eliminar" onClick={()=>eliminarPregunta(p.id_pregunta)}>
                        {/* papelera */}
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                          <path d="M3 6h18M9 6v-2a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                          <path d="M10 11v6M14 11v6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/>
        ...
        <section className="gs2-pane pane-right">
          ...
          <div className="gs2-pane-footer">
            <button className="gs2-btn gs2-btn-success w-100" onClick={crearPregunta}>
              Crear Pregunta
            </button>
          </div>
        </section>
          ...
          <div className="modal-dialog">
            <div className="modal-content p-4">
              ...
              <input
                ...
                onChange={(e)=>setEditSeccion({ ...editSeccion, nombre_seccion: e.target.value })}
              />
              <div className="mt-3" style={{ textAlign:"end" }}>
                <button className="gs2-btn gs2-btn-ghost" onClick={()=>setEditSeccion(null)}>Cancelar</button>
                <button className="gs2-btn gs2-btn-primary" onClick={guardarSeccionEditada}>Guardar</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="modal d-block" tabIndex="-1" style={{ background:"#0003" }}>
          <div className="modal-dialog modal-lg">
            <div className="modal-content p-4">
              ...
              />
              <div className="mt-3" style={{ textAlign:"end" }}>
                <button className="gs2-btn gs2-btn-ghost" onClick={()=>setEditPregunta(null)}>Cancelar</button>
                <button className="gs2-btn gs2-btn-primary" onClick={guardarPreguntaEditada}>Guardar cambios</button>
              </div>
            </div>
          </div>
        </div>
